...
function channelComparator(a: Channel, b: Channel, options: XmltvOptions): number {
    if (options.sortname) {
...
export function buildChannelsXml(data: GridApiResponse, optionOverrides: Partial<XmltvOptions> = {}): string {
    ...
    for (const channel of sortedChannels) {
        ...
        if (channel.thumbnail) {
            let src = channel.thumbnail.startsWith("http") ? channel.thumbnail : "https:" + channel.thumbnail;

            // New line to replace the hostname
            src = src.replace("zap2it.tmsimg.com", "ebyl.tmsimg.com");
        ...
        for (let i = 0; i < sortedEvents.length; i++) {
            ...
            if (event.program.originalAirDate || event.program.episodeAirDate) {
                ...
                if (!isNaN(airDate.getTime())) {
                    xml += `    <episode-num system="original-air-date">${
                        airDate.toISOString().replace("T", " ").split(".")[0]
                    }</episode-num>\n`;
                }
            }
